...
# input
alpha =         np.radians( -29.3)      #          np.radians(45)          # np.radians( -29.3)   #np.radians(0)      #
beta  =          np.radians(25.5)       #         np.radians(-45)            #np.radians(25.5)   # np.radians(0)      #
alpha2 =       np.radians( 30.7)        #          np.radians(0)           #np.radians( 30.7)    #np.radians(0)      #
beta2 =        np.radians(23.6)         #         np.radians(0)            #np.radians(23.6)    #np.radians(0)      #

# Angiogram #settings
d_s1 = 720#   900          # 900  # Source to patient was 720
d_sd1 =   976   #     1200  # Source to detector 1188 eerst
d_d1 = d_sd1 - d_s1  # Patient to detector
d_s2 = 720 #   900
d_sd2 =   976   #
d_d2 = d_sd2 - d_s2
d_p = 0.390625/1.3556 # 0.154    #        0.390625  # Pixel spacing
px = 512 -1    # Pixels (512)

def rotation(beta, alpha):
    M = np.array([(np.cos(beta), 0, np.sin(beta)),
                  (np.sin(alpha) * np.sin(beta), np.cos(alpha), np.sin(alpha) * -np.cos(beta)),
                  (-np.cos(alpha) * np.sin(beta), np.sin(alpha), np.cos(beta) * np.cos(alpha))])
    return M

def rotationT(alpha, beta):
...
M = rotationT(alpha,beta)
M2 = rotationT(alpha2,beta2)
