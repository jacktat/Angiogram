...
topmid2 = mapCenterline(idx2,[0, 512./2],h_flip=True,vert_flip=True)
r2_2 = (topmid2 - Detector2) / np.linalg.norm(topmid2 - Detector2)
midright2 = mapCenterline(idx2,[512./2, 0],h_flip=True,vert_flip=True)
...
P2 = mapCenterline(idx2,np.array([256,-256]))
Q2 = mapCenterline(idx2,np.array([256,256]))
R2 = mapCenterline(idx2,np.array([-256,256]))
SS_2 = mapCenterline(idx2,np.array([-256,-256]))
...
view2 = np.array([      #frame 133
  [                 -56.021 ,      111.41,            0],
  [                 -54.762 ,       102.6,       8.8844],
  [                 -53.503 ,      94.418,       17.147],
  [                 -50.356 ,      85.605,       26.487],
  [                 -50.356 ,      76.793,       35.282],
  [                 -50.356 ,      67.981,       44.077],
  [                 -50.356 ,       57.28,       54.756],
  [                 -49.097 ,      48.468,       63.641],
  [                 -50.356 ,      39.026,       73.148],
  [                 -52.244 ,      30.214,       82.142],
  [                 -53.503 ,      25.807,       86.716],
  [                 -54.762 ,      18.254,       94.358],
  [                 -55.392 ,      13.218,       99.423]
])
...
#second fit
legendre_coefs_x_proj2 = np.polynomial.legendre.legfit(ss2, proj2_2DX, 5) #Least squares fit of Legendre series to data, coefficients
x_coord_fit_proj2 = np.polynomial.legendre.legval(s, legendre_coefs_x_proj2) #Evaluate a Legendre series at points x
legendre_coefs_y_proj2 = np.polynomial.legendre.legfit(ss2, proj2_2DY, 5)
...
for p in range(len(legVx)):
    ...
    world_p2 = mapCenterline(idx2, temp_point_2, h_flip=True, vert_flip=True)
...
for iPoint in range(len(centerline1)): #range(len(centerline1))
    ...
    if len(sign_change_location) == 1:
        subList = []
        epi_curve_intersect3D = mapCenterline(idx2, sign_change_location[0])
        ...
        for i in range(len(sign_change_location)):
            epi_curve_intersect3D = mapCenterline(idx2,sign_change_location[i])
...
for sublist in reconstructed_points:
    if len(sublist) == 1:
        ...
        for k in range(len(sublist)):
            ax.scatter(*sublist[k], c='k', alpha=0.94)
...
plt.plot(legVx_2D, legVy_2D, color='k', label='Reparameterized Legendre Curve',alpha=0.5)
#ax2.scatter(legVx_2D[test_list[0]],legVy_2D[test_list[0]],c='r')
...
plt.xlim([-(px/2), (px/2)])
plt.ylim([-(px/2), (px/2)])
Image2 = mpimg.imread('frame133.png')
...
#ax3.scatter(*project3Dto2D(P2,SS_2,Q2,matches[0],px,d_p)[0],c='r')
