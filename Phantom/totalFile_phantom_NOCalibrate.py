...
def mapCenterline(idx, pixelPoint, h_flip = True, vert_flip = False):
    alpha = (dicom_instance.PositionerPrimaryAngle + dicom_instance.PositionerPrimaryAngleIncrement[idx] ) * -1
    beta =  (dicom_instance.PositionerSecondaryAngle +  dicom_instance.PositionerSecondaryAngleIncrement[idx] ) * -1
    M = np.identity(4)
    M[:3, :3] = (
    R.from_rotvec(np.deg2rad(alpha) * np.array([0, 1, 0])) *
    R.from_rotvec(np.deg2rad(beta) * np.array([1, 0, 0])) *
    R.from_rotvec(np.deg2rad(0) * np.array([0, 0, 1]))).as_matrix()
